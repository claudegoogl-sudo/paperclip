-- PLA-1889 — host-side half of PLA-1885: extends the per-binding egress
-- allowlist mechanism (PLA-723/731) onto a second keying axis, a plugin's own
...
--
-- Amendment A3 (CTO decision, PLA-1885): unlike 0138 — which flipped
...
-- vault's `serverUrl`, out of scope per PLA-1889 but harmless to backfill) is
...
-- consistent with A3: only ALREADY-CONFIGURED instances get the log-only
-- grace period).
