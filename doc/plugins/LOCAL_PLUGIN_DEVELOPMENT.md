...
- **Activation fails with `Worker initialize ... did not complete within 15000ms`.** That budget covers Node process startup, the worker bundle's module import, and your plugin's `setup()` — not just the last one. On a contended host or with a large bundle, the first two can exhaust it before your code runs, so do not assume `setup()` is at fault. The log line reports `elapsedSinceSpawnMs` alongside the configured budget; compare the two, and raise the budget with `PAPERCLIP_PLUGIN_INITIALIZE_TIMEOUT_MS` (milliseconds, positive, capped at 15 minutes; invalid values are ignored with a warning and fall back to the 15s default) when the host is the bottleneck.
