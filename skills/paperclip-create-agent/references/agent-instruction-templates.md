...
7. Adjacent variants intentionally diverge from the verbatim regions of the source template, so `agents-md-lint --against-template` is not the right gate. Run `agents-md-lint` without `--against-template` for structural checks, and call out the divergence in the hire comment.
8. Run the pre-submit checklist.

## How to apply the generic fallback

Use this when no template is close. Open `references/baseline-role-guide.md` and follow its section outline. That guide is structured so a CEO or hiring agent can produce a usable `AGENTS.md` without asking the board for prompt-writing help. After drafting, run `agents-md-lint` (without `--against-template`, since there is no canonical template to compare against) and the pre-submit checklist.
