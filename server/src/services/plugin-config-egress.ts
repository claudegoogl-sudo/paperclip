...
/**
 * PLA-1889 — host-side half of PLA-1885. Gates a plugin worker's
 ...
 * existing per-binding egress allowlist (PLA-723/731, `../handle-egress.ts`)
 * and its would-deny harvest (PLA-734, `./egress-harvest.ts`).
 *
 * CTO decision on PLA-1885, binding amendments:
 ...
 *    no run context — see PLA-1883). This is NOT a per-tenant boundary:
 *    flipping one company's `plugin_config_egress_allowlist` row to enforcing
 *    affects egress for the whole plugin.
 *  - A3 — rows backfilled by migration 0141 for already-installed instances
...
export async function enforcePluginConfigEgress(
  ...
  if (!decision.allow) {
    log.warn(
      ...
      "denied plugin ctx.http.fetch egress (PLA-1889)",
    );
...
// ---------------------------------------------------------------------------
// Operator review + set-allowlist + enforce-flip surface (PLA-1889, mirrors
// the PLA-735 pattern in secrets.ts for the per-binding allowlist). Routed
 ...
 * (same allowlist-poisoning guard as PLA-735).
 */
...
export async function enforcePluginConfigEgressAllowlist(
  ...
  log.info(
    ...
    "flipped plugin config-key egress allowlist row to enforcing — PLUGIN-WIDE effect per A2 (PLA-1889)",
  );
