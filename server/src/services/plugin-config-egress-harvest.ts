...
/**
 * PLA-1889 sibling of `egress-harvest.ts` (PLA-734) for the config-key
 ...
 * is company-agnostic (CTO amendment A2 on PLA-1885), so there is no
 * meaningful company to key an observation on either.
 */

/** Max distinct origins retained per plugin — same DoS-bound rationale as PLA-734. */
 ...
 * auto-apply any of them (allowlist-poisoning guard, matching PLA-735).
 */
