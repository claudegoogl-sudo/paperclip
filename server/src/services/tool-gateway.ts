          ...
          connectedMcpExecution
            ...
            ? await runWithTimeout(
                pluginToolDispatcher!.executeTool(
                  tool.name,
                  effectiveParameters,
                  // ToolRunContext.artifacts (PLA-574/PLA-888) is attached
                  // downstream by worker-rpc-host before the worker call, so the
                  // dispatch shape stays runId/agentId/companyId/projectId —
                  // matching the /tools/execute route.
                  {
                    agentId: session.agentId!,
                    runId: session.runId!,
                    companyId: session.companyId,
