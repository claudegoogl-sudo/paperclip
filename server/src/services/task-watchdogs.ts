...
// Fork: derive the terminal set from the shared heartbeat-run status model
// (so "succeeded_dirty" stays terminal) and union upstream's "interrupted"
// status, which is also a terminal (non-live) run for watchdog purposes.
const TASK_WATCHDOG_TERMINAL_RUN_STATUSES = [...TERMINAL_HEARTBEAT_RUN_STATUSES, "interrupted"] as const;
