...
/**
 * The single sanctioned carve-out env key: the operator-bound task_bridge
 * credential slot in an agent's adapterConfig env. Defined here — beside the
 * shared classifier — so the consumer path (heartbeat) and the auto-renewer
 * (task-bridge-renewal) reference ONE constant instead of two string copies.
 */
...
export function createTaskBridgeKeyClassifier(db: Db, companyId: string) {
  return async (resolvedKey: string, now?: Date): Promise<BridgeKeyVerifyResult> => {
    ...
    const [liveRow] = await db
      ...
      .limit(1);
    if (liveRow) {
      return classifyAgentApiKeyRow({ liveRow, anyRow: null, now });
    }
    ...
    const [anyRow] = await db
      ...
      .limit(1);
    return classifyAgentApiKeyRow({ liveRow: null, anyRow: anyRow ?? null, now });
  };
}
