...
export function toolAccessService(
  ...
  // Fork carryover for the plugin-backed connection health probe: the plugin's
  // configured endpoint (plugin_config.url) differs from the connection's own
  // config.url, so the health check needs the tools/list probe parameterized by
  // config override. Upstream's remoteTools() resolves the endpoint from
  // connection.config only, hence this variant, kept on the pre-upstream-refactor
  // remoteTools semantics.
  async function remoteToolsWithConfigOverride(
    connection: typeof toolConnections.$inferSelect,
  ...
  async function checkPluginConnectionHealth(
    ...
    if (configuredUrl.length > 0) {
      await remoteToolsWithConfigOverride(connection, { ...(connection.config ?? {}), url: configuredUrl });
  ...
  async function checkConnectionHealth(
    ...
    try {
      ...
      if (
        ...
        // Fork carryover: plugin-backed connections carry no url — evaluate
        // health against the per-company plugin config / runtime instead of
