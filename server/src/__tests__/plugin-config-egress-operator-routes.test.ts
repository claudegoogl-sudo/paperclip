/**
 * PLA-1889 — operator-only review + set-allowlist + enforce-flip HTTP routes
 * for the plugin config-key egress allowlist, against a real DB. Mirrors
 * PLA-735's `secret-egress-operator-routes.test.ts`.
 *
...
if (!support.supported) {
  console.warn(`Skipping PLA-1889 plugin config-egress operator route tests: ${support.reason ?? "unsupported environment"}`);
}
...
describeDb("PLA-1889 plugin config-egress operator routes", () => {
