/**
 * PLA-1889 — plugin config-key egress allowlist enforced at the `ctx.http.fetch`
 * chokepoint (`plugin-host-services.ts`, via `enforcePluginConfigEgress`).
 *
 * Pins the CTO amendments from PLA-1885 at the decision layer, against a real
 * DB:
...
if (!support.supported) {
  console.warn(`Skipping PLA-1889 plugin config-egress chokepoint tests: ${support.reason ?? "unsupported environment"}`);
}
...
describeDb("PLA-1889 plugin config-egress chokepoint", () => {
