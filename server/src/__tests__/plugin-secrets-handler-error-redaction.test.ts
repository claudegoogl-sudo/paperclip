/**
 * The plugin `secrets.resolve` host handler must never echo
 ...
 *
 * Constraint: no real leaked value is used — only synthetic,
...
describe("secrets.resolve source error redaction", () => {
...
describe("worker-manager host-error boundary redaction (AC2-B)", () => {
  ...
  // The boundary redactor now scrubs the fine-grained `github_pat_`
  ...
  it("scrubs the fine-grained github_pat_ shape at the boundary", () => {
  ...
  it("scrubs a lone github_pat_ that carries no other secret hint", () => {
